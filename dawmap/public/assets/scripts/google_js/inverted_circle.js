...
function displayMarker(circle, markers) {
    var rPosition = circle.get('position');
    var rDistance = circle.get('radius');
    rDistance = rDistance / 1000;
    for (var i=0; i<markers.length; i++) {
        var mPosition = markers[i].getPosition();
...
function InvertedCircle(opts) {
    ...
    this.set('resize_move', options.resize_move);
...
InvertedCircle.prototype.position_changed = function()
{
    ...
    if(this.get('sizer_left') && this.get('sizer_right') && this.get('sizer_up') && this.get('sizer_down') && this.get('sizer_move')){
        ...
        //var move_endpoint = google.maps.geometry.spherical.computeOffset(this.getCenter(), this.getRadius(), 0);
        ...
        //this.get('sizer_move').setPosition(move_endpoint);
    }
};
...
InvertedCircle.prototype.radius_changed = function()
{
  ...
  if(this.get('sizer_left') && this.get('sizer_right') && this.get('sizer_up') && this.get('sizer_down')){
    ...
    var right_endpoint = google.maps.geometry.spherical.computeOffset(this.getCenter(), this.getRadius(), 90);
    var up_endpoint = google.maps.geometry.spherical.computeOffset(this.getCenter(), this.getRadius(), 360);
    var down_endpoint = google.maps.geometry.spherical.computeOffset(this.getCenter(), this.getRadius(), 360);
    this.get('sizer_left').setPosition(left_endpoint);
    this.get('sizer_right').setPosition(right_endpoint);
    this.get('sizer_up').setPosition(up_endpoint);
    this.get('sizer_down').setPosition(down_endpoint);
...
InvertedCircle.prototype.addSizer_ = function() {
    ...
    var sizer_icon_move = new google.maps.MarkerImage(this.get('resize_move'));
    ...
    var sizer_move = new google.maps.Marker({
        position: map.getCenter(),
        title: 'Move me!',
        raiseOnDrag: false,
        cursor: 'move',
        icon: sizer_icon_move
    });
    ...
    sizer_move.bindTo('map', this, 'map');
    sizer_move.bindTo('visible', this, 'editable');
    sizer_move.bindTo('draggable', this, 'editable');
    
    this.set('sizer_left', sizer_left);
    this.set('sizer_right', sizer_right);
    this.set('sizer_up', sizer_up);
    this.set('sizer_down', sizer_down);
    this.set('sizer_move', sizer_move);
    ...
    google.maps.event.addListener(sizer_right, 'drag', function() {
        ...
        displayMarker(me, markers);
    });
    
    google.maps.event.addListener(sizer_move, 'dragstart', function() {
        var old_radius = me.getRadius();
        me.setRadius(old_radius);
        me.setCenter(me.get('sizer_move').getPosition());
    });
    
    google.maps.event.addListener(sizer_move, 'dragend', function() {
        var old_radius = me.getRadius();
        me.setRadius(old_radius);
        me.setCenter(me.get('sizer_move').getPosition());
        displayMarker(me, markers);
